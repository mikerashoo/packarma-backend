    ...
    public function index(Request $request)
    {
        ...
        try
        {
            ...
            if($token)
            {
                ...
                $data = CustomerNotificationHistory::select('id','page_name','title','body','created_at')->where([['status','1'],['user_id', $user_id]]);
                ...
                {
                    errorMessage(__('customer_notification_histories.notification_not_found'), $msg_data);
                }
                if(isset($request->search) && !empty($request->search)) {
                    $data = fullSearchQuery($data, $request->search,'title|body');
                }
                if ($defaultSortByName) {
                    $orderByArray = ['customer_notification_history.title' => 'ASC'];
                }
